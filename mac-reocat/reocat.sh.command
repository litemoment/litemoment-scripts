...
# Script Description:
# concat denuxer multiple REOCAM files into one mp4 video.
...
while true; do
    read -rp "Proceed with folder '$VIDEODIR'?, t for time from REOCAMfilename (y/n/t): " yn
...
if [ -d "$IN_DIR" ]; then
  ...
  if $filename_datetime_set; then
	  ...
	  if [[ $filename =~ ^RecM0([0-9]{1})_(DST)?([0-9]{8})_([0-9]{6}) ]]; then
	      datetime_string="${BASH_REMATCH[3]}${BASH_REMATCH[4]}"
